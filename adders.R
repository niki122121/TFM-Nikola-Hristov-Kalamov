...
# función auxiliar, combina el output de 32 bytes de la función digest en un número grande y lo normaliza en un valor de 0 a 1 
# para ser representado gráficamente
raw_to_bigz = function(rawInpt_vec){
  ...
  # valor máximo usado para normalizar el resto (en este caso se trata de 2^256 - 1)
  valor_norm = as.bigz("0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff") - 1
  return(resultVec/valor_norm)
}
...
distribucion = 0.5 #recta escogida como distribución simple
sumadores = distribucion - datos_hash
...
# plot de los resultasdos, se ve que la gráfica de los datos en sí (plot izquierdo arriba) no se aprecia el cambio realizado por 
# el actor deshonesto (se ve que los valores originales en azul se corresponded casi identicamente con los
# valores modificados en rojo). Sin embargo, en la gráfica del blockchain (arriba derecha) se observa como no solo el décimo
...
# En la parte de abajo se añaden los SUMADORES al blockchain original y se realiza otra comparación con la misma secuencia modificada

# secuencias blockchain junto a los sumadores
datos_hash_sumados = datos_hash + sumadores
datos_cambiados_hash_sumados = datos_cambiados_hash + sumadores

# se observa como la secuencia sumada (azul) se ajusta perfectamente a la distribución escogida (la recta y=0.5) 
# mientras que de nuevo la secuencia blockchain modificada (rojo) divierge a partir del décimo valor
par(mfrow=c(1,1))
plot(x, datos_hash_sumados, col="blue", ylim=c(-0.2,1))
lines(x, datos_hash_sumados, type = "b", lty = 1, col = "blue")
points(x, datos_cambiados_hash_sumados, col = "red", pch="*")
lines(x, datos_cambiados_hash_sumados, type = "b", lty = 2, col = "red", pch="*")
